...
def process_pair(i, j, graph_list, graph_networkx_list):
    ...
    common_gene_overlap = [
        find_neighbor_overlap(G1, G2, item, item) for item in genes_common
    ]
    ...
    s1_nei_gene = np.array(
        [
            np.random.choice(list(G1.neighbors(gene1)))
            if len(list(G1.neighbors(gene1))) != 0
            else gene1
            for gene1 in s1_gene
        ]
    )
    s2_nei_gene = np.array(
        [
            np.random.choice(list(G2.neighbors(gene1)))
            if len(list(G2.neighbors(gene1))) != 0
            else gene1
            for gene1 in s2_gene
        ]
    )
